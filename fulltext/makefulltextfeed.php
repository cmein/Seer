...
foreach ($items as $key => $item) {
	...
	*/
		...
		if (isset($effective_url)) {
			$newitem->addElement('dc:identifier', $effective_url);
		} else {
			$newitem->addElement('dc:identifier', $item->get_permalink());
		}
...
if ($options->caching || $format == 'json') {
	...
	if ($format == 'json') {
		...
		// valid ns prefixed elements do not appear in array/object view
		// so we rewrite them (we only use DC and MediaRSS) for JSON output...
		$jsonrss->rss = @simplexml_load_string(str_replace(array('<dc:', '</dc:', '<media:', '</media:'), array('<dc_', '</dc_', '<media_', '</media_'), $output));
		$output = json_encode($jsonrss);
		header("Content-type: application/json; charset=UTF-8");
	}
	if ($html_only && $item_count == 0) {
		// do not cache - in case of temporary server glitch at source URL
	} elseif ($options->caching) {
...
function convert_to_utf8($html, $header=null)
{
	...
	if ($html || $header) {
		...
		if (!$encoding) {
			// search for encoding in HTML - only look at the first 2000 characters
