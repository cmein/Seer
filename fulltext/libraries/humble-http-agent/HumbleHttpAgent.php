...
class HumbleHttpAgent
{
	...
	protected function getUserAgent($url, $asArray=false) {
		...
		if ($host && isset($this->userAgentMap[$host])) {
			$ua = $this->userAgentMap[$host];
		} else {
				...
				while (count($urls) > 0) {
					...
					foreach ($subset as $orig => $url) {
						...
						*/
							...
							$httpRequest->addheaders(array('Referer' => 'http://www.google.co.uk/url?sa=t&source=web&cd=1'));
			...
			while (count($urls) > 0) {
				...
				foreach ($subset as $orig => $url) {
					...
					*/
						...
						// add referer for picky sites
						$headers[] = 'Referer: http://www.google.co.uk/url?sa=t&source=web&cd=1';
						// send cookies, if we have any
						if ($cookies = $this->cookieJar->getMatchingCookies($req_url)) {
							$this->debug("......sending cookies: $cookies");
		...
		// sequential (file_get_contents)
			...
			foreach ($urls as $orig => $url) {
				...
				*/
					...
					// add referer for picky sites
					$httpContext['http']['header'] .= 'Referer: http://www.google.co.uk/url?sa=t&source=web&cd=1'."\r\n";
					if ($cookies = $this->cookieJar->getMatchingCookies($req_url)) {
						$this->debug("......sending cookies: $cookies");
